...
class Pooled(FixtureMaker):
    def get_generator(self):
        ...
        for pool in pools:
            if len(pool) % 2 != 0:
                pool += [BYE]
        for i, _ in enumerate([i for i in pools[1] if i != BYE]):
            fixtures = []
            for pool in pools:
                mid = len(pool) // 2
                for j in range(mid):
        ...
        games = [
            Game(p1, p2, self.tournament) for p1, p2 in zip(pool_one, pool_two[:2])
        ]
        if BYE in pool_one and BYE in pool_two:
            pool_one = [i for i in pool_one if i != BYE]
            pool_two = [i for i in pool_two if i != BYE]
        yield games + [
            Game(p1, p2, self.tournament) for p1, p2 in zip(pool_one[2:], pool_two[2:])
        ]
