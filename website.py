...
def tournament_specific(app, comps: dict[str, Tournament]):
    ...
    def stats_site(tournament, team_name):
        ...
        players = [
            (i.name, [(k, v) for k, v in i.get_stats().items()]) for i in team.players
        ]
        return (
            render_template(
    ...
    def game_editor(tournament, game_id):
        ...
        team_one_players = [
            ((1 - i), v) for i, v in enumerate(teams[0].players)
        ]
        team_two_players = [
            ((1 - i), v) for i, v in enumerate(teams[1].players)
        ]
        if key is None:
            return (
                render_template(
...
def universal_tournament(app, comps: dict[str, Tournament]):
    ...
    def universal_stats_site(team_name):
        ...
        players = [
            i
            for i in get_all_players(comps)
            if i.nice_name() in [j.nice_name() for j in team.players]
        ]

        players = [
            (i.name, [(k, v) for k, v in i.get_stats().items()]) for i in players
        ]
        return (
            render_template(
                "tournament_specific/each_team_stats.html",
