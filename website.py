...
from structure.AllTournament import get_all_teams, get_all_players, get_all_officials, get_all_games
...
def init_api(app, comps: dict[str, Tournament]):
    ...
    def log():
        ...
        if key is None:
            return render_template("tournament_specific/game_editor/no_access.html",
                                   error="This page requires a password to access:"), 403
        elif key != admin_password:
            return render_template("tournament_specific/game_editor/no_access.html",
                                   error="The password you entered is not correct"), 403
...
def tournament_specific(app, comps: dict[str, Tournament]):
    ...
    def home_page(tournament):
        in_progress = any([not (i.best_player or i.bye) for i in comps[tournament].games_to_list()])
        ladder = sorted(comps[tournament].teams, key=lambda a: (-a.games_won, -(a.get_stats()["Point Difference"])))
        ongoing_games = [i for i in comps[tournament].games_to_list() if i.in_progress()]
        current_round = [game for r in comps[tournament].finals for game in r] if comps[tournament].in_finals else \
            comps[tournament].fixtures[-1]
        if all([i.bye for i in current_round]) and len(
                comps[tournament].fixtures) > 1:  # basically just for home and aways
            current_round = comps[tournament].fixtures[-2]
        players = []
        for i in comps[tournament].teams:
            players += i.players
        ...
        return render_template("tournament_home.html", tourney=comps[tournament], ongoing=ongoing_games,
                               current_round=current_round, players=players, notes=notes, in_progress=in_progress,
                               tournament=f"{tournament}/", ladder=list(enumerate(ladder, start=1))), 200

    @app.get('/<tournament>/fixtures/')
    def fixtures(tournament):
        fixtures = comps[tournament].fixtures
        finals = comps[tournament].finals
        fixtures = [(n, [i for i in j if not i.bye or i.best_player]) for n, j in
                    enumerate(fixtures)]
        finals = [(n, [i for i in j if not i.bye or i.best_player]) for n, j in
                  enumerate(finals)]
        fixtures = [i for i in fixtures if i[1]]
        finals = [i for i in finals if i[1]]
        return render_template("tournament_specific/site.html", fixtures=fixtures, finals=finals,
    ...
    def stats_directory_site(tournament):
        teams = [(i.name, i.nice_name()) for i in sorted(comps[tournament].teams, key=lambda a: a.nice_name())]
        return render_template("tournament_specific/stats.html", teams=teams, tournament=f"{tournament}/"), 200
    ...
    def stats_site(tournament, team_name):
        ...
        for i in comps[tournament].games_to_list():
            if team not in [j.team for j in i.teams] or i.bye:
                continue
            if i.started:
                recent_games.append((repr(i) + f" ({i.score_string()})", i.id, i.tournament.nice_name()))
            else:
                upcoming_games.append((repr(i) + f" ({i.score_string()})", i.id, i.tournament.nice_name()))
        players = [(i.name, [(k, v) for k, v in i.get_stats().items()]) for i in team.players]
        return render_template("tournament_specific/each_team_stats.html",
                               stats=[(k, v) for k, v in team.get_stats().items()],
                               teamName=team.name,
                               players=players, teamNameClean=team.nice_name(), recent_games=recent_games,
    ...
    def game_site(tournament, game_id):
        ...
        for i in get_all_games(comps):
            if not all([k.nice_name() in [j.team.nice_name() for j in i.teams] for k in game.teams]): continue
            if i == game: continue
            prev_matches.append((f"{repr(i)} ({i.score_string()}) [{i.tournament.name}]", i.id, i.tournament))
        ...
        return render_template("tournament_specific/game_page.html", game=game, status=status,
                               players=[i.tidy_name() for i in players],
                               teams=teams, stats=stats, player_stats=player_stats, official=game.primary_official,
                               commentary=game_string_to_commentary(game), best=best, roundNumber=round_number,
    ...
    def ladder_site(tournament):
        ...
        teams = [(i.name, i.nice_name(), [(v, priority[k]) for k, v in i.get_stats().items()]) for i in
                 sorted(comps[tournament].teams, key=lambda a: (-a.games_won, -(a.get_stats()["Point Difference"])))]
        headers = [(i, priority[i]) for i in (["Team Names"] + [i for i in comps[tournament].teams[0].get_stats()])]
        return render_template("tournament_specific/ladder.html",
                               headers=[(i - 1, k, v) for i, (k, v) in enumerate(headers)],
                               teams=teams, tournament=f"{tournament}/"), 200
    ...
    def players_site(tournament):
        ...
        players = [(i.name, i.team.nice_name(), [(v, priority[k]) for k, v in i.get_stats().items()]) for i in
                   all_players]
        headers = ["Name"] + [i for i in comps[tournament].teams[0].players[0].get_stats()]
        return render_template("tournament_specific/players.html",
                               headers=[(i - 1, k, priority[k]) for i, k in enumerate(headers)],
                               players=players, tournament=f"{tournament}/"), 200
    ...
    def official_site(tournament, nice_name):
        ...
        return render_template("tournament_specific/official.html", name=official.name, link=official.nice_name(),
                               stats=[(k, v) for k, v in official.get_stats().items()], games=recent_games,
                               tournament=f"{tournament}/"), 200
    ...
    def official_directory_site(tournament):
        ...
        return render_template("tournament_specific/all_officials.html", officials=official,
                               tournament=f"{tournament}/"), 200
    ...
    def game_editor(tournament, game_id):
        ...
        if key is None:
            ...
            return render_template("tournament_specific/game_editor/no_access.html",
                                   error="The password you entered is not correct"), 403
        if not game.started:
            return render_template("tournament_specific/game_editor/game_start.html",
                                   players=[i.tidy_name() for i in players],
                                   teams=teams, game=game, tournament=f"{tournament}/"), 200
        elif not game.game_ended():
            return render_template("tournament_specific/game_editor/edit_game.html",
                                   players=[i.tidy_name() for i in players], teamOnePlayers=team_one_players,
                                   teamTwoPlayers=team_two_players,
                                   teams=teams, game=game, tournament=f"{tournament}/"), 200
        elif not game.best_player or key == admin_password:
            return render_template("tournament_specific/game_editor/finalise.html",
                                   players=[i.tidy_name() for i in players],
                                   teams=teams, game=game, tournament=f"{tournament}/"), 200
        else:
            return render_template("tournament_specific/game_editor/game_done.html",
                                   error="This game has already been completed!"), 400

    @app.get('/<tournament>/games/create')
    def create_game(tournament):
        if not comps[tournament].fixtures_class.manual_allowed():
            return render_template("tournament_specific/game_editor/game_done.html",
                                   error="This competition cannot be edited manually!"), 400
        elif any([not (i.best_player or i.bye) for i in comps[tournament].games_to_list()]):
            return render_template("tournament_specific/game_editor/game_done.html",
                                   error="There is already a game in progress!"), 400
        teams = comps[tournament].teams
        next_id = comps[tournament].fixtures[-1][-1].id if comps[tournament].fixtures else 0
        ...
        if key is None:
            ...
            return render_template("tournament_specific/game_editor/create_game_teams.html",
                                   tournament=f"{tournament}/",
                                   officials=officials,
                                   teams=teams, id=next_id), 200
    ...
    def create_game_players(tournament):
        ...
        if key is None:
            ...
            return render_template("tournament_specific/game_editor/create_game_players.html",
                                   tournament=f"{tournament}/",
                                   officials=officials,
                                   players=players, id=next_id), 200


def universal_tournament(app, comps: dict[str, Tournament]):
    @app.get('/teams/')
    def universal_stats_directory_site():
        teams = [(i.name, i.nice_name()) for i in sorted(get_all_teams(comps), key=lambda a: a.nice_name())]
        return render_template("tournament_specific/stats.html", teams=teams, tournament=""), 200
        ...
        for c in comps.values():
            for i in c.games_to_list():
                if team_name not in [j.team.nice_name() for j in i.teams] or i.bye:
                    continue
                if i.started:
                    recent_games.append((repr(i) + f" ({i.score_string()}) [{c.name}]", i.id, i.tournament.nice_name()))
                else:
                    upcoming_games.append(
                        (repr(i) + f" ({i.score_string()}) [{c.name}]", i.id, i.tournament.nice_name()))
    ...
    def universal_ladder_site():
        ...
        teams = [(i.name, i.nice_name(), [(v, priority[k]) for k, v in i.get_stats().items()]) for i in
                 sorted(get_all_teams(comps), key=lambda a: (-a.games_won, -(a.get_stats()["Point Difference"])))]
        headers = [(i, priority[i]) for i in (["Team Names"] + [i for i in get_all_teams(comps)[0].get_stats()])]
        return render_template("tournament_specific/ladder.html",
                               headers=[(i - 1, k, v) for i, (k, v) in enumerate(headers)],
                               teams=teams, tournament=""), 200
    ...
    def universal_players_site():
        ...
        return render_template("tournament_specific/players.html",
                               headers=[(i - 1, k, priority[k]) for i, k in enumerate(headers)],
                               players=players, tournament=""), 200

    @app.get('/officials/<nice_name>/')
    def universal_official_site(nice_name):
        official = [i for i in get_all_officials(comps) if i.nice_name() == nice_name][0]
        recent_games = []
        for i in get_all_games(comps):
            if official != i.primary_official:
                continue
    ...
    def universal_official_directory_site():
        ...
        return render_template("tournament_specific/all_officials.html", officials=official, tournament=""), 200
